          ...
          />
{
  hasInput && (
    inputType === 'password' ? (
      <FontAwesomeIcon icon={ faEyeSlash } onClick={togglePasswordVisibility} className="cursor-pointer absolute right-2 top-9"/>
      ) : (
        <FontAwesomeIcon icon={ faEye } onClick={togglePasswordVisibility} className="cursor-pointer absolute right-2 top-9"/>
      ) 
  )
}
          {errors.password && (
          ...
          {location.pathname === '/signup' ? 
            (
              <fieldset>
                ...
                <span className="text-red text-xsm">{errors.passwordConfirm.message}</span>
                )}
              </fieldset>
            ) : (null)
          }
